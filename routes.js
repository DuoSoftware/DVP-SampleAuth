...
exports.getAToken = function(req,res) {
    ...
    var model = module.exports,
        ...
        db = require("mongojs")(databaseUrl, collections);
            ...
            var fs = require("fs");
        fs.readFile('views/authenticate.html',function(err,data) { //fs.readFile('views/authenticate.html',function(err,data) {
            if (err || !data) {
                console.log(err);
            }
            res.end(data);
...
exports.postClientRegister = function(req,res) {
    var redirectUri = req.body.redirect_uri || "",userName=req.body.username||"",
        databaseUrl = "oauth",
        collections = ["users",'oauth_clients','oauth_client_scope'],
        db = require("mongojs")(databaseUrl, collections);
    ...
    db.users.find({username:userName},function(err,users) {
        ...
        if (users && users.length > 0) {
            ...
            var tenant = users[0].tenant;

            db.oauth_clients.find({client_id: clientId},function(err,client) {
                if (!client || !client.length) {
                    db.oauth_client_scope.save({client_id : clientId, client_scope: scope, client_secret : clientSecret,tenant:tenant,company:company},function(err,saved) {
                        if (!err) {
                            db.oauth_clients.save({client_id : clientId, redirect_uri: redirectUri, client_secret : clientSecret},function(err,saved) {
                                if (!err) {
                                    ...
                                    res.end(JSON.stringify({msg: err, status:'failure'}))
                                }
                            })
                        }
                        else {
            ...
            res.end(JSON.stringify({msg: "invalid User Information's", status:'failure'}))
        }
    });
...
exports.postRegister = function(req,res) {
    ...
    var tenant = req.body.tenant;
        ...
        res.setHeader('Content-Type', 'application/json');
            if (username  && password && password.length > 2) {//&& username.match('^[A-Za-z0-9-_\^]{5,30}$')
            db.users.find({username:username},function(err,users) {
                console.log(users);
                if (!users || !users.length) {
                    db.users.save({username: username, password: password,tenant:tenant,company:company}, function(err,saved) {
                        if (!err) {
...
exports.confirmDialog = function(req,res) {
